...
function generateTable() {
	...
	document.getElementById("aircraftrow").value = playerShips[0]['row'];
	document.getElementById("battleshiprow").value = playerShips[1]['row'];
	document.getElementById("submarinerow").value = playerShips[2]['row'];
	document.getElementById("destroyerrow").value = playerShips[3]['row'];
	document.getElementById("patrolboatrow").value = playerShips[4]['row'];
	
	document.getElementById("aircraftcolumn").value = playerShips[0]['column'];
    document.getElementById("battleshipcolumn").value = playerShips[1]['column'];
	document.getElementById("submarinecolumn").value = playerShips[2]['column'];
	document.getElementById("destroyercolumn").value = playerShips[3]['column'];
	document.getElementById("patrolboatcolumn").value =playerShips[4]['column'];

	document.getElementById("aircraftalign").value = playerShips[0]['orientation'];
	document.getElementById("battleshipalign").value = playerShips[1]['orientation'];
	document.getElementById("submarinealign").value = playerShips[2]['orientation'];
	document.getElementById("destroyeralign").value = playerShips[3]['orientation'];
	document.getElementById("patrolboatalign").value = playerShips[4]['orientation'];
...
function checkSink(whichGrid) {
	...
	if (whichGrid == "player") {
		...
		if (foundAircraftCarrier == 0 && playerAircraftCarrierSunk == 0) {
			...
			document.getElementById("gamestatus").value += "\n Player Destroyer Sunk.";
		}
		else if (foundPatrol == 0 && playerPatrolSunk == 0) {
			playerPatrolSunk = 1;
			document.getElementById("gamestatus").value += "\n Player Patrol Boat Sunk.";
		}
		if (playerAircraftCarrierSunk == 1 && playerBattleshipSunk == 1 && playerSubmarineSunk == 1 && playerDestroyerSunk == 1 && playerPatrolSunk == 1) {
			...
			$.ajax({
				type: "GET",
				url: "http://battledeploy.aws.af.cm/users/grid.php?won=computer",
				data: "",
				success: function()
				{							
		...
		if (computerAircraftCarrierSunk == 1 && computerBattleshipSunk == 1 && computerSubmarineSunk == 1 && computerDestroyerSunk == 1 && computerPatrolSunk == 1) {
			...
			$.ajax({
				type: "GET",
				url: "http://battledeploy.aws.af.cm/users/grid.php?won=player",
				data: "",
				success: function()
				{							
...
function generateManTable(a1,a2,b1,b2,s1,s2,d1,d2,p1,p2,aalign,balign,salign,dalign,palign) {
	...
	for (var row = 0; row < computerCollisionTable.length; row++) {
		...
		playerShips[0]['row'] = a2;
	playerShips[0]['column'] = a1;
	playerShips[0]['orientation'] = aalign;

	playerShips[1]['row'] = b2;
	playerShips[1]['column'] = b1;
	playerShips[1]['orientation'] = balign;

	playerShips[2]['row'] = s2;
	playerShips[2]['column'] = s1;
	playerShips[2]['orientation'] = salign;

	playerShips[3]['row'] = d2;
	playerShips[3]['column'] = d1;
	playerShips[3]['orientation'] = dalign;

	playerShips[4]['row'] = p2;
	playerShips[4]['column'] = p1;
	playerShips[4]['orientation'] = palign;
	
	console.log(playerShips[0]['column']);
		console.log(playerShips[1]['column']);
	console.log(playerShips[2]['column']);
	console.log(playerShips[3]['column']);
	console.log(playerShips[4]['column']);
	
		console.log(playerShips[0]['row']);
		console.log(playerShips[1]['row']);
	console.log(playerShips[2]['row']);
	console.log(playerShips[3]['row']);
	console.log(playerShips[4]['row']);
	
	
		console.log(playerShips[0]['orientation']);
		console.log(playerShips[1]['orientation']);
	console.log(playerShips[2]['orientation']);
	console.log(playerShips[3]['orientation']);
	console.log(playerShips[4]['orientation']);
	...
	for (i = 0; i < ships; i++) {
		...
		if (playerShips[i].orientation == "horizontal") {
			...
			do {
				...
				/*for (j = 0; j < playerShips[i].length; j++) {
					if (playerCollisionTable[playerShips[i].row + j][playerShips[i].column] != 0) { // A collision occurred.
						console.log((playerShips[i].row + j),layerShips[i].column);
						collisionFlag = 1;
					}
				}*/
	...
	for (i = 0; i < ships; i++) {
		...
		if (computerShips[i].orientation == "horizontal") {
			do {
				...
				for (j = 0; j < computerShips[i].length; j++) {
					if (computerCollisionTable[computerShips[i].row][computerShips[i].column + j] != 0) { // A collision occurred.
						console.log((playerShips[i].row),(layerShips[i].column + j));
						collisionFlag = 1;
					}
				}
...
// JavaScript for generating the game grid.
function showManTable() {
	var gridDiv = document.getElementById("tableDiv");
	
	var aircraftrow = document.getElementById("aircraftrow").value;
	var battleshiprow = document.getElementById("battleshiprow").value;
	var submarinerow = document.getElementById("submarinerow").value;
	var destroyerrow = document.getElementById("destroyerrow").value;
	var patrolboatrow = document.getElementById("patrolboatrow").value;
	
	var aircraftcolumn = document.getElementById("aircraftcolumn").value;
	var battleshipcolumn = document.getElementById("battleshipcolumn").value;
	var submarinecolumn = document.getElementById("submarinecolumn").value;
	var destroyercolumn = document.getElementById("destroyercolumn").value;
	var patrolboatcolumn = document.getElementById("patrolboatcolumn").value;

	var aircraftalign = document.getElementById("aircraftalign").value;
	var battleshipalign = document.getElementById("battleshipalign").value;
	var submarinealign = document.getElementById("submarinealign").value;
	var destroyeralign = document.getElementById("destroyeralign").value;
	var patrolboatalign = document.getElementById("patrolboatalign").value;

	gridDiv.innerHTML = generateManTable(aircraftcolumn,aircraftrow,battleshipcolumn,battleshiprow,submarinecolumn,submarinerow,destroyercolumn,destroyerrow,patrolboatcolumn,patrolboatrow,aircraftalign,battleshipalign,submarinealign,destroyeralign,patrolboatalign);
	gridClickHandlers();
	sunk();
}