...
if(isset($_POST['update']))
{
	...
	if(empty($err))
	{
		$update = "UPDATE ".USERS." SET full_name = '".filter($_POST['fullname'])."', user_name = '".filter($_POST['username'])."', usr_email = AES_ENCRYPT('".filter($_POST['email'])."', '$salt'), user_level = '".filter($_POST['user_level'])."', photo='".$pic."'";
...
if(isset($_POST['add_user']))
{
	...
	if(empty($err))
	{
		...
		$email_to = $new_user_email; $email_from=GLOBAL_EMAIL;$password = $pw; $subj = $mssg;
...
<div class="well">
	...
	<h2>Welcome !! <?php echo $_COOKIE['fullname']; ?>!, Manage your users</h2>
	...
	?>
	<table cellpadding="5" cellspacing="5" border="0" width="100%" class="table table-hover">
	<tr>
	...
	<tr>
		...
		<td id="form_<?php echo $r['id']; ?>" style="display:none;">
			...
			<tr>
			<td>Username</td>
			<td><input type="text" name="username" value="<?php echo $r['user_name']; ?>" readonly/></td>
			</tr>
			<tr>
			<td>Email</td>
			<td><input type="text" name="email" value="<?php echo $r['email']; ?>" readonly/></td>
			</tr>
			<tr>
			<td>New Password</td>
			...
			<td>
				...
				<?php
				$levels = array('User' => 1, 'Administrator' => 4);
	...
	<form name="create_new" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="new_user_form" class="form-horizontal">
	...
	<tr>
	<td colspan="2">
		<input type="submit" name="add_user" class="btn btn-info " value="Create User" />
	</td>
	</tr>
	</table>
	</form>
